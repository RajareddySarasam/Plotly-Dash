...
app.layout = html.Div([
    dcc.Graph(figure=px.histogram(df, x='day', y='tip'), id='bar-graph',clickData={}),
...
def update_text(chosen_data):  # the function argument comes from the component property of the Input
    ...
    print(dff.head())
    fig = px.pie(dff, names='sex', values='total_bill',  title=f'total bill by sex on {extract_day}')
